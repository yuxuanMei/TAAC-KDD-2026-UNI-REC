...
_FALLBACK_MODEL_CFG = {
    ...
    'use_predict_rule_calibrator': False,
    'predict_rule_scale': 0.02,
    'time_zone_offset_hours': 0.0,
    'use_semantic_rule_features': False,
    'semantic_rule_direct_scale': 1.0,
...
def build_model(
    ...
    if 'item_rule_fids_info' in sig.parameters:
        item_info = []
        for fid in [5, 6, 7, 8, 10, 12, 16, 83, 84, 85]:
            try:
                offset, length = dataset.item_int_schema.get_offset_length(fid)
            except KeyError:
                continue
            vs = max(dataset.item_int_vocab_sizes[offset:offset + length])
            item_info.append((int(fid), int(offset), int(length), int(vs)))
        kwargs['item_rule_fids_info'] = item_info
...
def main() -> None:
    ...
    unique_user_ids = len(set(all_user_ids))
    duplicate_user_ids = len(all_user_ids) - unique_user_ids
    logging.info(
        "Inference complete: raw_rows=%s, unique_user_ids=%s, "
        "duplicate_user_id_rows=%s",
        len(all_probs),
        unique_user_ids,
        duplicate_user_ids,
    )
    ...
    logging.info(
        "Prediction JSON contains %s keys after user_id de-duplication",
    ...
    logging.info(
        "Saved %s raw predictions to %s (json_keys=%s, size=%s bytes, %.2f MB)",
        len(all_probs),
        output_path,
        len(predictions["predictions"]),
        output_size,
    ...
    if cache_path:
        ...
        try:
            ...
            logging.info(
                "Backed up predictions to %s (size=%s bytes, %.2f MB)",
                backup_path,
                backup_size,
