...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        din_branch_loss_weight: float = 0.0,
    ) -> None:
        ...
        self.din_branch_loss_weight: float = float(din_branch_loss_weight)
        ...
        logging.info(f"PCVRHyFormerRankingTrainer loss_type={loss_type}, "
            ...
            f"din_branch_loss_weight={self.din_branch_loss_weight}")

    def _sync_ema_reinitialized_params(self, param_names: List[str]) -> None:
        """Copy only freshly reinitialized model parameters into EMA."""
        if not param_names:
            return
        ...
        with torch.no_grad():
            for name in param_names:
                src = model_params.get(name)
                dst = ema_params.get(name)
        ...
        if missing:
            logging.warning(
                "EMA reinit sync missed %d params, examples=%s",
                len(missing), missing[:5])
        logging.info(
            "Synced %d reinitialized high-cardinality Embeddings into EMA "
            "without resetting other EMA weights",
            synced)
    ...
    def train(self) -> None:
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            if epoch >= self.reinit_sparse_after_epoch and self.sparse_optimizer is not None:
                ...
                reinit_ptrs, reinit_param_names = self.model.reinit_high_cardinality_params(
                    self.reinit_cardinality_threshold)
                self._sync_ema_reinitialized_params(reinit_param_names)
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        if self.loss_type == 'focal':
            loss_cls = sigmoid_focal_loss(logits, smooth_label, alpha=self.focal_alpha, gamma=self.focal_gamma)
        else:
            loss_cls = F.binary_cross_entropy_with_logits(logits, smooth_label)

        loss = loss_cls
        if self.din_branch_loss_weight > 0:
            din_logits = getattr(self.model, 'last_din_logits', None)
            if din_logits is not None:
                din_logits = din_logits.squeeze(-1)
                if self.loss_type == 'focal':
                    loss_din = sigmoid_focal_loss(
                        din_logits, smooth_label,
                        alpha=self.focal_alpha, gamma=self.focal_gamma)
                else:
                    loss_din = F.binary_cross_entropy_with_logits(din_logits, smooth_label)
                loss = loss + self.din_branch_loss_weight * loss_din
