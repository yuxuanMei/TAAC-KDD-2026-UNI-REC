...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--rare16_profile', action='store_true', default=False,
                        help='Append lightweight rare item16/OOV profile '
                             'features to the existing user_dense token.')
    parser.add_argument('--use_oov_residual_calibrator', action='store_true',
                        default=False,
                        help='Learn a small bounded logit residual from OOV, '
                             'recency, history-match, and time-window signals. '
                             'This replaces hard test-time OOV penalties.')
    parser.add_argument('--oov_residual_scale', type=float, default=0.05,
                        help='Maximum absolute logit scale of '
    ...
    parser.add_argument('--use_abs_time_ns', action='store_true', default=False,
                        help='Inject sample-level absolute Beijing-time periodic '
                             'features into all NS tokens.')
    parser.add_argument('--use_session_crossday_time', action='store_true', default=False,
                        help='Inject sequence-level session and cross-day time '
                             'buckets into behavior tokens.')
    parser.add_argument('--use_light_din_branch', action='store_true', default=False,
                        help='Add a small DIN-style side branch using item tokens '
                             'as query over seq_c tokens.')
    parser.add_argument('--din_branch_loss_weight', type=float, default=0.0,
                        help='Optional auxiliary loss weight for the light DIN branch.')
    ...
    parser.add_argument('--seq_causal', action='store_true', default=False,
                        help='Whether the LongerEncoder self-attention uses a causal mask '
                             '(only effective when --seq_encoder_type=longer)')
    parser.add_argument('--seq_semilocal_causal_mask', action='store_true', default=False,
                        help='Use a recent-first semi-local causal mask in each '
                             'sequence self-attention encoder.')
    parser.add_argument('--seq_semilocal_window', type=int, default=128,
                        help='Window size for --seq_semilocal_causal_mask.')
...
def main() -> None:
    ...
    train_loader, valid_loader, pcvr_dataset = get_pcvr_data(
        ...
        rare16_profile=args.rare16_profile,
    ...
    def _item_fids_info(fids):
        ...
        for fid in fids:
            try:
                ...
                continue
            vs = pcvr_dataset.item_int_vocab_by_fid.get(int(fid), -1)
    ...
    model_args = {
        ...
        "item_rare_fids_info": _item_fids_info([5, 6, 7, 8, 10, 12, 16, 84, 85]),
        "item_match_fids_info": _item_fids_info([5, 6, 16, 83, 84, 85]),
        ...
        "use_oov_residual_calibrator": args.use_oov_residual_calibrator,
        "oov_residual_scale": args.oov_residual_scale,
        "time_zone_offset_hours": args.time_zone_offset_hours,
        "ns_tokenizer_type": args.ns_tokenizer_type,
        "user_ns_tokens": args.user_ns_tokens,
        "item_ns_tokens": args.item_ns_tokens,
        "use_abs_time_ns": args.use_abs_time_ns,
        "use_session_crossday_time": args.use_session_crossday_time,
        "use_light_din_branch": args.use_light_din_branch,
        "seq_semilocal_causal_mask": args.seq_semilocal_causal_mask,
        "seq_semilocal_window": args.seq_semilocal_window,
    }
    ...
    for fid, offset, length in pcvr_dataset.item_int_schema.entries:
        if fid == 16:
            fid16_offset = offset
            fid16_vs = pcvr_dataset.item_int_vocab_by_fid.get(16, -1)
            break
    model_args["fid16_offset"] = fid16_offset
    model_args["fid16_vs"] = fid16_vs
    ...
    trainer = PCVRHyFormerRankingTrainer(
        ...
        din_branch_loss_weight=args.din_branch_loss_weight,
    )
